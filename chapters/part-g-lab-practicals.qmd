...
**❓ Question 1:** Looking at the plot, does the new protocol appear more effective?
...
**💡 Teaching Point:** When any expected cell count is < 5, chi-square approximations break down. Fisher's exact test is mandatory.
...
cat("   → Odds of pain relief are", round(or_estimate, 1), 
    "times higher with new protocol\n\n")

cat("2. 95% CI: [", round(or_ci_lower, 2), ",", 
    round(or_ci_upper, 2), "]\n")
cat("   → Wide CI reflects small sample size (n=16)\n")
cat("   → CI excludes 1, consistent with p<0.05\n\n")
...
**❓ Question 2:** What happens to the chi-square p-value? Is it similar to Fisher's?

**💡 Teaching Point:** With small cell counts:
...
- Statistical significance ≠ clinical certainty
- Replication needed
...
Using the same code structure, analyze this 2×2 table:
...
# 🧪 Lab Practical 2: Bootstrap Confidence Intervals
...
**💡 Teaching Point:** Skewed data violate normality assumptions for t-tests. Bootstrap makes no distributional assumptions!
...
cat("⚠️ Problem: This CI assumes normality!\n")
cat("   With skewed data and n=15, this assumption is violated.\n")
```

**❓ Question 1:** Why can't we easily get a parametric CI for the median?
...
**💡 Teaching Point:** The bootstrap distribution approximates the sampling distribution of the statistic!
...
**❓ Question 2:** Is the CI symmetric around the median? Why or why not?
...
**💡 Teaching Point:** BCa intervals are theoretically superior but require more computation. With small samples, differences are often modest.
...
**❓ Question 3:** Which method gives the narrowest CI? Why might that be misleading?
...
✅ **Minimum resamples:**
- Percentile CI: 1,000–5,000 resamples
- BCa CI: 2,000–10,000 resamples
...
**Tasks:**
1. Plot the distribution—is it symmetric?
...
# 🧪 Lab Practical 3: Mann-Whitney U Test
...
**💡 Teaching Point:** Outliers and skewness violate t-test assumptions. Mann-Whitney uses ranks, making it robust!
...
**❓ Question 1:** Even if normality tests pass, why might Mann-Whitney be preferred here?
...
- Note the "confidence interval" reported—this is for the shift parameter

**💡 Teaching Point:** The CI is for the Hodges-Lehmann estimate (pseudo-median difference), not the difference in medians!
...
**❓ Question 2:** How does the Hodges-Lehmann estimate compare to the difference in medians?
...
cat("Hodges-Lehmann estimate:", hl_estimate, "days\n\n")
cat("→ They're close but not identical!\n")
```
...
if (abs(effect_size$effsize) < 0.3) {
  cat("→ Effect size is SMALL\n")
} else if (abs(effect_size$effsize) < 0.5) {
  cat("→ Effect size is MEDIUM\n")
} else {
  cat("→ Effect size is LARGE\n")
}
```
...
**❓ Question 3:** What happens if we remove the outliers (31 and 42)? Would the t-test be more appropriate?
...
- Large-sample approximation for n ≥ 30
...
# 🧪 Lab Practical 4: Wilcoxon Signed-Rank Test
...
**💡 Teaching Point:** Paired data show individual change—much more powerful than comparing independent groups!
...
**❓ Question 1:** Why do we focus on differences rather than before/after separately?
...
**💡 Teaching Point:** Wilcoxon signed-rank tests whether the distribution of differences is symmetric around zero.
...
if (abs(effect_size$effsize) > 0.5) {
  cat("→ LARGE effect: Mindfulness program shows substantial benefit\n")
} else if (abs(effect_size$effsize) > 0.3) {
  cat("→ MEDIUM effect: Mindfulness program shows moderate benefit\n")
} else {
  cat("→ SMALL effect: Mindfulness program shows modest benefit\n")
}
```
...
**❓ Question 2:** When would the two tests disagree?
...
**💡 Teaching Point:** Wilcoxon is robust to outliers—one reason to prefer it over paired t-test!
...
2. **What does "V statistic" mean in Wilcoxon signed-rank?**
   ...
   - Large V → most differences are positive
   - Small V → most differences are negative
...
1. Create a contingency table and visualize the technique×success relationship
...
1. **Acceptability Thresholds:** Is α = 0.78 acceptable for a pilot scale with n=25?
...
6. Examine "alpha if item deleted" – would removing any item increase alpha?
...
1. Calculate power for a paired t-test with n=25, d=0.50, α=0.05
...
library(tidyverse)
library(mice)
if (requireNamespace("naniar", quietly = TRUE)) library(naniar)  # For missing data visualization
if (requireNamespace("VIM", quietly = TRUE)) library(VIM)     # For missing data patterns
...
5. Inspect imputed values using `densityplot()` – are they plausible?
...
cat("univariate outliers using z-scores (|z| > 2.5) and Tukey fences (1.5×IQR),\n")
cat("multivariate outliers using Mahalanobis distance (χ² p < .001), and\n")
