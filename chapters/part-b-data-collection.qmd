...
Little's MCAR test evaluates whether missingness is consistent with the MCAR mechanism. The test compares observed means across missing-data patterns; a large p-value suggests MCAR is plausible, whereas a small p-value indicates that missingness likely depends on observed data (i.e., not MCAR). With very small samples the test has limited power, so interpret results alongside descriptive checks.

```{r}
# Little's MCAR test using the BaylorEdPsych package
if (requireNamespace("BaylorEdPsych", quietly = TRUE)) {
  library(BaylorEdPsych)
  mcar_result <- LittleMCAR(study_data %>% select(age, satisfaction, performance))
  mcar_result$ChiSquare
  mcar_result$df
  mcar_result$p.value
} else {
  cat("Install 'BaylorEdPsych' to run Little's MCAR test; with very small samples, complement the test with descriptive checks.\n")
}
```

Interpretation: A non-significant p-value (e.g., p > 0.05) supports the MCAR assumption. However, with small *n*, the test may fail to detect systematic missingness, so combine it with substantive reasoning and comparisons of observed covariates between complete and incomplete cases.
