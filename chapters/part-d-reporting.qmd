...
By the end of this chapter, you will be able to compute and interpret standardised effect sizes (Cohen's d, odds ratios, correlation coefficients), report confidence intervals alongside point estimates, explain why p-values alone are insufficient for interpreting small-sample findings, and distinguish between statistical significance and practical importance. You will understand how to frame results in terms of magnitude and precision rather than dichotomous significance.

### The Limitations of P-Values

P-values indicate the probability of observing data as extreme as (or more extreme than) what was observed, assuming the null hypothesis is true. They do not measure effect size, practical importance, or the probability that the null hypothesis is true. With small samples, p-values are especially problematic:
...
Standardised effect sizes express differences or associations on a common scale, facilitating comparison across studies and meta-analysis. Common standardised effect sizes include:
...
### Unstandardised Effect Sizes and Original Units

Standardised effect sizes are useful for comparison, but unstandardised effect sizes (differences in original units) are often more interpretable for applied audiences. For example, "Method A students scored 10 points higher on average (95% CI: 7 to 13 points)" is more tangible than "Cohen's d = 0.8 (95% CI: 0.5 to 1.1)".
